...
///
#define HX8353E_WIDTH  102
#define HX8353E_HEIGHT 160
...
Screen_HX8353E::Screen_HX8353E() {
    _pinReset          = Board_SPI_LCD_RST;
    _pinDataCommand    = Board_SPI_LCD_DC;
    _pinChipSelect     = Board_SPI_LCD_CS;
    _pinBacklight      = NULL;
}
...
void Screen_HX8353E::invert(bool flag)
{
    _writeCommand(flag ? HX8353E_INVON : HX8353E_INVOFF);
}
void Screen_HX8353E::setBacklight(bool flag)
{
    if (_pinBacklight!=NULL) digitalWrite(_pinBacklight, flag);
}
void Screen_HX8353E::setDisplay(bool flag)
{
...
void Screen_HX8353E::_fastFill(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t colour)
{
    ...
    for (uint32_t t=(uint32_t)(y2-y1+1)*(x2-x1+1); t>0; t--) {
        SPI_Driver.transfer(hi);
        SPI_Driver.transfer(lo);
    }
...
void Screen_HX8353E::_writeData16(uint16_t data16)
{
    digitalWrite(_pinDataCommand, HIGH);
    digitalWrite(_pinChipSelect, LOW);
    SPI_Driver.transfer(highByte(data16));
    SPI_Driver.transfer(lowByte(data16));
    digitalWrite(_pinChipSelect, HIGH);
}
void Screen_HX8353E::_writeData88(uint8_t dataHigh8, uint8_t dataLow8)
{
    digitalWrite(_pinDataCommand, HIGH);
    digitalWrite(_pinChipSelect, LOW);
    SPI_Driver.transfer(dataHigh8);
    SPI_Driver.transfer(dataLow8);
    digitalWrite(_pinChipSelect, HIGH);
}
