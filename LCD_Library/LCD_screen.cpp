...
void LCD_screen::showInformation(uint16_t x0, uint16_t y0)
{
    ...
    y0 += fontSizeY()+1;
    setFontSize(1);
    gText(x0, y0, "release " + std::to_string(LCD_SCREEN_RELEASE/100) + "." + std::to_string(LCD_SCREEN_RELEASE/10%10) + std::to_string(LCD_SCREEN_RELEASE%10));
    y0 += fontSizeY()+1;
    ...
    y0 += fontSizeY()+1;
    gText(x0, y0, "Pixels:   " + std::to_string(screenSizeX()) + " x " + std::to_string(screenSizeY()));
    y0 += fontSizeY()+1;
    gText(x0, y0, "Readable: " + (isReadable()) ? "yes" : "no");
    y0 += fontSizeY()+1;
    gText(x0, y0, "Touch:    " + (isTouch()) ? "yes" : "no");
    y0 += fontSizeY()+1;
    gText(x0, y0, "Storage:  " + (isStorage()) ? "yes" : "no");
    y0 += fontSizeY()+1;*/
}
...
void LCD_screen::line(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t colour)
{
    if ((x1 == x2) && (y1 == y2)) {
        ...
        uint8_t flag = abs(wy2 - wy1) > abs(wx2 - wx1);
        if (flag) {
            _swap(wx1, wy1);
            _swap(wx2, wy2);
        }
